...
class TestGame2048Helper {
    ...
    private fun testMerge(initialValues: List<String?>, expectedValues: List<String>) {
        val mergedValues = initialValues.moveAndMerge { char: String -> char.repeat(2) }

        Assert.assertEquals("Wrong result after moving and merge: \n$initialValues",
                expectedValues,
                mergedValues)
    }
}